...
import { fetchLiveGallery } from "@/lib/api";
...
export function GallerySection() {
  ...
  useEffect(() => {
    fetchLiveGallery().then((liveList) => {
      if (liveList && liveList.length > 0) {
        const mapped = liveList.map((g) => ({
          id: typeof g.id === "number" ? g.id : parseInt(String(g.id)) || 1,
          type: (g.type as "photo" | "video") || "photo",
          title: g.title,
          date: g.date,
          img: g.img,
          gridClass: g.grid_class || g.gridClass || "md:col-span-1 md:row-span-1 h-[300px] md:h-[400px]",
          description: g.description,
        }));
        setAllItems(mapped as GalleryItem[]);
      }
    });
  }, []);
