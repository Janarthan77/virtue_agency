...
export default function AdminPortal() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [enqRes, statsRes, statusRes] = await Promise.all([
            ...
            <div className="p-2.5 rounded-xl bg-white border border-slate-200 text-xs space-y-1.5 shadow-2xs">
              <div className="flex items-center justify-between text-slate-600">
                <span className="flex items-center gap-1.5 font-normal">
                  <Server size={13} className="text-blue-500" /> Backend API
                </span>
                <span className="text-emerald-700 font-semibold font-mono text-xs">Port 5001</span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
                <span className="flex items-center gap-1.5 font-normal">
          ...
          <div className="flex-1 min-h-0 bg-white border border-slate-200/90 rounded-xl overflow-hidden shadow-2xs flex flex-col">
            {loading ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                ...
                <p className="text-slate-500 text-sm">Fetching enquiries from Node.js Backend &amp; Supabase...</p>
              </div>
            ) : enquiries.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
